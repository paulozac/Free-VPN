...
public class WireGuardAdapter {
    ...
    class var backendVersion: String {
        guard let ver = awgVersion() else { return "unknown" }
    ...
    public func getRuntimeConfiguration(completionHandler: @escaping (String?) -> Void) {
        workQueue.async {
            ...
            if let settings = awgGetConfig(handle) {
    ...
    private func setupLogHandler() {
        ...
        awgSetLogger(context) { context, logLevel, message in
    ...
    private func startWireGuardBackend(wgConfig: String) throws -> Int32 {
        ...
        let handle = awgTurnOn(wgConfig, tunnelFileDescriptor)
        if handle < 0 {
